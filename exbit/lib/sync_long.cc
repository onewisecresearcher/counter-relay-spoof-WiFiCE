...
int general_work (int noutput, gr_vector_int& ninput_items,
	...
	case SYNC:
		...
		d_fir.filterN(d_correlation, in, std::min(SYNC_LENGTH, std::max(ninput - 127, 0))); //for ac

		//while(i + 63 < ninput) {
		while(i + 127 < ninput) {  // for ac
	...
	case COPY:
		while(i < ninput && o < noutput) {
			...
			//int rel = d_offset + 192 - d_frame_start; //192 = 160 (short) + 32 (GI) 
			int rel = d_offset + 384 - d_frame_start; // for ac // 384 = 320 (short) + 64 (GI) 
			int rellong = d_offset - d_frame_start;
...
/* ----------- start: this is for 802.11a ----------------------------------------
			// Notes how swapping happens in a

			//[0-159] --> short, [160-191] -> GI [192-319] --> long

			//we want, out[0-127] as LONG, so in[192 to 319] goes first
			//we want, out[128-288] as SHORT, so in[0-159] goes next
			//we get rid of GI, so no use of in[160-191]

			
			if(rel >= 0 && (rel < 320 || ((rel - 320) % 80) > 15)) {
				if(o >=0 && o<128){
					out[o] = in_delayed[i+192] * exp(gr_complex(0, d_offset * d_freq_offset));
					//std::cout << "1RX Preamble Sample from Long_in" << o << " : "<< in_delayed[o] << std::endl;
					//std::cout << "1RX Preamble Sample from Long_out" << o << " : "<< out[o] << std::endl;
				}
				if(o >=128 && o<288){
					out[o] = in_delayed[i-128] * exp(gr_complex(0, d_offset * d_freq_offset));
					//std::cout << "2RX Preamble Sample from Long_in" << o << " : "<< in_delayed[o] << std::endl;
					//std::cout << "2RX Preamble Sample from Long_out" << o << " : "<< out[o] << std::endl;
				}

				// This part is for capturing the short in 802.11ac -> the VHT-STF
				// Location of VHT-STF -> in[560 to 639]
				//if(o >=288 && o<368){
					//out[o] = in_delayed[i+560] * exp(gr_complex(0, d_offset * d_freq_offset));
					//std::cout << "3RX Preamble Sample from Long_in" << o << " : "<< in_delayed[o] << std::endl;
					//std::cout << "3RX Preamble Sample from Long_out" << o << " : "<< out[o] << std::endl;
				}
				o++;
				
			} 

-----------------------------------end: this is for 802.11a ------------------------------ */

// start: this is for 802.11ac
			// Notes how swapping happens in ac

			//[0-319] --> short, [320-383] -> GI [384-639] --> long

			//we want, out[0-255] as LONG, so in[384 to 639] goes first
			//we want, out[256-575] as SHORT, so in[0 to 319] goes next
			//we get rid of GI, so no use of in[320-383]

			//std::cout << "Long_in" << i << " : "<< in_delayed[i] << std::endl;

/* existing start
			if(rel >= 0 && (rel < 640 || ((rel - 640) % 160) > 31)) {
				if(o >=0 && o<256){
					out[o] = in_delayed[i+384] * exp(gr_complex(0, d_offset * d_freq_offset));
					//std::cout << "1RX Preamble Sample from Long_in" << o << " : "<< in_delayed[o] << std::endl;
					//std::cout << "1RX Preamble Sample from Long_out" << o << " : "<< out[o] << std::endl;
				}
				if(o >=256 && o<576){
					out[o] = in_delayed[i-256] * exp(gr_complex(0, d_offset * d_freq_offset));
					//p = i -256;
					//std::cout << "2RX Preamble Sample from Long_in" << o << " : "<< in_delayed[o] << std::endl;
					std::cout << "2RX Preamble Sample from Long_out" << o << " : "<< out[o] << std::endl;
				}
			...
			//if(rel >= 0 && (rel < 640 || ((rel - 640) % 160) > 31)) {
				if(o >=0 && o<256){
					//out[o] = in_delayed[i+384+32] * exp(gr_complex(0, d_offset * d_freq_offset));
					out[o] = in_delayed[i+384] * exp(gr_complex(0, d_offset * d_freq_offset));
					//std::cout << "1RX Preamble Sample from Long_in" << o << " : "<< in_delayed[o] << std::endl;
					//std::cout << "1RX Preamble Sample from Long_out" << o << " : "<< out[o] << std::endl;
				}
				if(o >=256 && o<576){
					//out[o] = in_delayed[i-256+32] * exp(gr_complex(0, d_offset * d_freq_offset));
					out[o] = in_delayed[i-256] * exp(gr_complex(0, d_offset * d_freq_offset));
					//p = i -256;
					//std::cout << "2RX Preamble Sample from Long_in" << o << " : "<< in_delayed[o] << std::endl;
					//std::cout << "2RX Preamble Sample from Long_out" << o << " : "<< out[o] << std::endl;
...
void forecast (int noutput_items, gr_vector_int &ninput_items_required) {
	...
	if(d_state == SYNC) {
		...
		ninput_items_required[0] = 128;// for ac
		ninput_items_required[1] = 128;// for ac
	...
	for(int i = 0; i < 3; i++) {
		for(int k = i + 1; k < 4; k++) {
			...
			if(diff == 128) { // for ac
				...
				d_freq_offset = arg(first * conj(second)) / 128; //for ac
				// nice match found, return immediately
				return;

			//} else if(diff == 63) {
			} else if(diff == 127) { //for ac
				d_frame_start = min(get<1>(vec[i]), get<1>(vec[k]));
				//d_freq_offset = arg(first * conj(second)) / 63;
				d_freq_offset = arg(first * conj(second)) / 127; //for ac
			//} else if(diff == 65) {
			} else if(diff == 129) { // for ac
				d_frame_start = min(get<1>(vec[i]), get<1>(vec[k]));
				//d_freq_offset = arg(first * conj(second)) / 65;
				d_freq_offset = arg(first * conj(second)) / 129; // for ac
			}
		}
	}
}
