...
decode_mac_impl(bool log, bool debug) :
	block("decode_mac",
			...
			gr::io_signature::make(1, 1, 108), // for ac
...
int general_work (int noutput_items, gr_vector_int& ninput_items,
	...
	while(i < ninput_items[0]) {
		...
		if(copied < d_frame.n_sym) {
			...
			std::memcpy(d_rx_symbols + (copied * 108), in, 108); // for ac
			copied++;
...
void decode() {
	...
	for(int i = 0; i < d_frame.n_sym * 108; i++) { // for ac
...
private:
	...
	uint8_t d_rx_symbols[108 * MAX_SYM];// for ac
