...
frame_equalizer_impl::frame_equalizer_impl(Equalizer algo, double freq, double bw, bool log, bool debug) :
	gr::block("frame_equalizer",
			gr::io_signature::make(1, 1, 128 * sizeof(gr_complex)), //changed to 128 from 64
			gr::io_signature::make(1, 1, 108)), //ac number of usable subcarriers 108
...
frame_equalizer_impl::general_work (int noutput_items,
	...
	gr_complex symbols[108];//for ac
	//gr_complex current_symbol[64];
	gr_complex current_symbol[128]; //for ac

	dout << "FRAME EQUALIZER: input " << ninput_items[0] << "  output " << noutput_items << std::endl;

	//for(int i = 0; i < 384; i++) {
	//		std::cout << "FFT out"<< i<< ", in: "<< in[i] << std::endl;
	//	}
	...
	while((i < ninput_items[0]) && (o < noutput_items)) {
		...
		std::memcpy(current_symbol, in + i*128, 128*sizeof(gr_complex)); //for ac
		...
		//for ac
		for(int i = 0; i < 128; i++) {
			current_symbol[i] *= exp(gr_complex(0, 2*M_PI*d_current_symbol*160*(d_epsilon0 + d_er)*(i-64)/128));
		}

// start of pilot calculation
...
/*
		...
		er *= d_bw / (2 * M_PI * d_freq * 160);// for ac
...
// end of pilot calculation
		...
		//d_equalizer->equalize(current_symbol, d_current_symbol,
				//symbols, out + o * 108, d_frame_mod); // for ac

		d_equalizer->equalize(current_symbol, d_current_symbol,
				symbols, out + o * 48, d_frame_mod); // for ac
...
/* testing
		...
		if(d_current_symbol == 2) {
			...
			if(decode_signal_field(out + o * 108)) {// for ac
		...
		if(d_current_symbol > 2) {
			...
			message_port_pub(pmt::mp("symbols"), pmt::cons(pmt::make_dict(), pmt::init_c32vector(108, symbols)));// for ac
		}
testing*/
